...
int main(){
  ...
  for (int del_theta = 0; del_theta <360; del_theta++){
    x1_s.push_back(10*cos(PI*del_theta/180));
    y1_s.push_back(10*sin(PI*del_theta/180));
    x2_s.push_back(6*cos(PI*del_theta/180));
    y2_s.push_back(6*sin(PI*del_theta/180));
  }
  plt::figure_size(500, 500);
  plt::plot(x1_s,y1_s,"yo-");
  plt::plot(x2_s,y2_s,"ro-");
  plt::text(12, 12, "Sun");
  plt::xlim(-200, 200);
  plt::ylim(-200, 200);
  plt::title("I'm learning how to plot");
  plt::show();
  plt::clf();

  for (int N_days=1; N_days <= 365; N_days++){
      x_e.push_back(earth.pos_x/1e9);
      y_e.push_back(earth.pos_y/1e9);
      // cout << N_days << "\t";
      // printf("x: %f\t", earth.pos_x/1e9);     // Units: Millions of Km
      // printf("y: %f\t", earth.pos_y/1e9);     // Units Millions of Km
      // printf("V_x: %f\t", earth.vel_x/1e3);   // Units: Km/s
      // printf("V_y: %f\n", earth.vel_y/1e3);   // Units: Km/s
      earth.step(dt, &sun);
  }
